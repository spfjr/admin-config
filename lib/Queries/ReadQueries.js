...
class ReadQueries extends Queries {
    ...
    getFilteredReferenceData(references, rawValues) {
        ...
        for (let i in references) {
            let reference = references[i],
                ...
                referenceUrl = !reference.url() ? reference.name() : reference.url();

            for (let k in identifiers) {
                calls.push(getOne(targetEntity, 'listView', identifiers[k], referenceUrl));
            }
        }
    ...
    getOptimizedReferenceData(references, rawValues) {
        ...
        for (let i in references) {
            ...
            calls.push(getRawValues(targetEntity, targetEntity.name() + '_ListView', 'listView', 1, reference.perPage(), singleCallFilters, {}, reference.sortField(), reference.sortDir(), reference.url()));
        }
    ...
    getReferencedListData(referencedLists, sortField, sortDir, entityId) {
        ...
        for (let i in referencedLists) {
            ...
            calls.push(getRawValues(targetEntity, viewName, 'listView', 1, referencedList.perPage(), filter, {}, currentSortField, currentSortDir, referencedList.url()));
        }
